...
def load_df():
    ...
    # Always (re)derive lists at load
...
# --- SAFETY: re-derive helper lists on the filtered view (prevents KeyError) ---
...
if use_primary:
    ...
    if risk_sel:   view = view[view["risk_primary"].str.lower().isin({r.lower() for r in risk_sel})]
    if vend_sel:   view = view[view["vendor_primary"].str.lower().isin({v.lower() for v in vend_sel})]
...
if not view.empty:
    ...
    show_cols += ["vendor_primary","risk_primary","risk_score"] if use_primary else ["vendors","risks"]
    st.dataframe(
        tbl[show_cols],
        use_container_width=True,
        column_config={
